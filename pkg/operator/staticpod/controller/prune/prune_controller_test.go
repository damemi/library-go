...
func TestSyncRevisionLimits(t *testing.T) {
	groupVersionResource := schema.GroupVersionResource{Group: "openshiftapiserver.operator.openshift.io", Version: "v1alpha1", Resource: "openshiftapiserveroperatorconfigs"}
	tests := []struct {
		name              string
		...
		{
			name: "updates limits appropriately",
			gvr:  groupVersionResource,
			config: newUnstructured(
				map[string]interface{}{
					"operatorRevisionLimit": map[string]interface{}{"succeeded": "2", "failed": "5"},
				}),
			succeededLimit:    0,
			failedLimit:       0,
			expectedSucceeded: 2,
		...
		{
			name: "defaults to 0 for unknown/non-int values",
			gvr:  groupVersionResource,
			config: newUnstructured(
				map[string]interface{}{
					"operatorRevisionLimit": map[string]interface{}{"succeeded": "2pac", "failed": "5"},
				}),
			succeededLimit:    0,
			failedLimit:       0,
			expectedSucceeded: 0,
			expectedFailed:    5,
			expectedErr:       "error converting succeeded revision limit 2pac to int: strconv.Atoi: parsing \"2pac\": invalid syntax",
		},

		{
			name: "defaults to 0 for unset values",
			gvr:  groupVersionResource,
			config: newUnstructured(
				map[string]interface{}{}),
			succeededLimit:    0,
		...
		{
			name: "invalid updates don't affect current valid limits",
			gvr:  groupVersionResource,
			config: newUnstructured(
				map[string]interface{}{
					"operatorRevisionLimit": map[string]interface{}{"succeeded": "xyz", "failed": "5"},
				}),
			succeededLimit:    5,
			failedLimit:       3,
			expectedSucceeded: 5,
			expectedFailed:    5,
			expectedErr:       "error converting succeeded revision limit xyz to int: strconv.Atoi: parsing \"xyz\": invalid syntax",
		},
	}

	for _, test := range tests {
		dynamicClient := dynamicfake.NewSimpleDynamicClient(runtime.NewScheme(), test.config)
		c := &PruneController{
			succeededRevisionLimit: test.succeededLimit,
			failedRevisionLimit:    test.failedLimit,
			dynamicClient:          dynamicClient,
			operatorConfigGVR:      test.gvr,
		...
		if c.succeededRevisionLimit != test.expectedSucceeded {
			t.Errorf("expected succeeded revision limit %d, got %d", test.expectedSucceeded, c.succeededRevisionLimit)
		}
		if c.failedRevisionLimit != test.expectedFailed {
			t.Errorf("expected failed revision limit %d, got %d", test.expectedFailed, c.failedRevisionLimit)
		}
	}
}

func newUnstructured(spec map[string]interface{}) *unstructured.Unstructured {
